...
IVFPQindex::~IVFPQindex()
{
	...
	if(m_imgLocationOri != NULL)
	{
		delete[] m_imgLocationOri;
		m_imgLocationOri = NULL;
	}
...
void IVFPQindex::LoadIndex(string srcFile)
{
	...
	else
	{
		...
		for(int i=0; i<m_coarseK; i++)
		{
            ...
            if(num>0)
            {
			    m_ivfList[i].resize(num);
			    for(int j = 0; j < num; j++)
			    {
			    	fin.read((char*)&m_ivfList.at(i).at(j).videoId, sizeof(int));
			    	m_ivfList.at(i).at(j).PQindex.resize(m_pq_m);
			    	for(int k = 0; k < m_pq_m; k++)
					{
						fin.read((char*)&m_ivfList.at(i).at(j).PQindex.at(k), sizeof(uchar));
					}
			    }
		...
		m_imgLocationOri = new ImgNameStruct[m_imgNum];
		for(int i=0;i<m_imgNum;i++)
		{
			fin.read(m_imgLocationOri[i].ptr, sizeof(char)*max_path);
		}
		fin.close();
	}
}
...
void IVFPQindex::IndexDatabaseInsert(vector<string> featFiles)
{

	cout<<"Index database...."<<endl;
	m_ivfList.resize(m_coarseK);
	
	int num = min(int(featFiles.size())+m_imgNum,m_maxIndexNum);
	m_imgLocation = new ImgNameStruct[num+m_imgNum];
	for(int i = 0; i < m_imgNum; i++)
	{
		memset(m_imgLocation[i].ptr, 0, max_path * sizeof(char));
		strcpy(m_imgLocation[i].ptr, m_imgLocationOri[i].ptr);
	}

	for(int i = 0; i < num - m_imgNum; i++)
	{
		struct timeval starttime, endtime;
		double timeuse;
		gettimeofday(&starttime, NULL);
...
void IVFPQindex::SaveIndex(string desDir)
{
	...
	string m_desDir = desDir + "/PQ_Index_db_" + strImgNum +"_dim_" + strDim + "_k_" + strK +"_PQ_m"+str_pq_m + "_k"+str_pq_k+ ".fvecs";
